  ...
  useEffect(() => {
    if (open && editingItem) {
      setFormData({
        ...
        provider_id: editingItem.provider?.id,
      });
    }
  }, [open, editingItem]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      ...
      <DialogContent sx={{ p: 0, bgcolor: "#f8f9fa" }}>
        ...
        >
          <Box sx={{ flex: 6.5, p: 3, borderRight: { md: "1px solid #e0e0e0" } }}>
            <Typography
              variant="h6"
              fontWeight={600}
              mb={2}
          ...
          >
            ...
            <Box>
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                ...
                >
                  <Chip
                    label={editingItem?.type?.name || "N/A"}
                    color="info"
                    size="small"
                    variant="filled"
                    sx={{ fontWeight: "bold" }}
            ...
            <Box sx={{ flexGrow: 1 }}>
              <Typography
                ...
                sx={{ display: "flex", alignItems: "center", gap: 1 }}
              >
                <Storefront fontSize="small" /> Nhà cung cấp
              </Typography>
              ...
              <Paper variant="outlined" sx={{ p: 0, borderRadius: 2, overflow: "hidden" }}>
                ...
                >
                  <Avatar
                    src={editingItem?.provider?.logo_url}
                    sx={{
                      width: 48,
                      height: 48,
                  ...
                  <Box>
                    <Typography variant="subtitle1" fontWeight={700} lineHeight={1.2}>
                      {editingItem?.provider?.name}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      @{editingItem?.provider?.username}
                    </Typography>
                  </Box>
                </Box>
                ...
                <Stack spacing={1.5} sx={{ p: 2 }}>
                  <Box sx={{ display: "flex", gap: 1.5 }}>
                    <Email fontSize="small" color="action" sx={{ mt: 0.3 }} />
                    <Typography variant="body2" sx={{ wordBreak: "break-all" }}>
                      {editingItem?.provider?.email}
                    </Typography>
                  </Box>

                  <Box sx={{ display: "flex", gap: 1.5 }}>
                    <Phone fontSize="small" color="action" sx={{ mt: 0.3 }} />
                    <Typography variant="body2">{editingItem?.provider?.phone}</Typography>
                  </Box>

                  {editingItem?.provider?.website_link && (
                    <Box sx={{ display: "flex", gap: 1.5 }}>
                      <Language fontSize="small" color="action" sx={{ mt: 0.3 }} />
                      <Typography variant="body2">{editingItem?.provider?.website_link}</Typography>
                    </Box>
                  )}

                  <Box sx={{ display: "flex", gap: 1.5 }}>
                    <MapIcon fontSize="small" color="action" sx={{ mt: 0.3 }} />
                    <Typography variant="body2">
                      {editingItem?.provider?.address}, {editingItem?.provider?.province}
                    </Typography>
                  </Box>
                </Stack>
              </Paper>
